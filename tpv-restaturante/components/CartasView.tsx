...
function Toggle({ value, onChange, label, color, C }: ToggleProps) {
  return (
    <button
...
export default function CartasView({ catalog, onSave, colors: C }: {
  ...
  function updateCategory(idx: number, field: string, value: unknown) {
    const next = { ...local };
  ...
  function updateProduct(pid: string, field: string, value: unknown) {
    const next = { ...local };
  ...
  function toggleAllergen(pid: string, aid: string) {
              ...
              <div key={cat.id || ci} className="mb-3 rounded-xl overflow-hidden" style={{ border: `1px solid ${C.line}` }}>
                ...
                <div className="flex items-center gap-2 px-3 py-2" style={{ background: C.surface }}>
                  ...
                  <Toggle
                    ...
                    C={C}
                  />
                  <button onClick={() => setSearchQuery(cat.name)} style={{ color: C.muted }} className="p-1 hover:opacity-80">
...
function ModificadoresTab({ colors: C }: { colors: Theme }) {
  const [groups, setGroups] = useState<ModifierGroup[]>([]);
  const [productMods, setProductMods] = useState<Record<string, string[]>>({});
  ...
  const updateGroup = (id: string, key: string, val: unknown) => {
    setGroups(prev => prev.map(g => g.id === id ? { ...g, [key]: val } as ModifierGroup : g));
  };
  ...
  const updateOption = (groupId: string, optId: string, key: string, val: unknown) => {
    setGroups(prev => prev.map(g => {
      if (g.id !== groupId) return g;
      return { ...g, options: g.options.map(o => o.id === optId ? { ...o, [key]: val } as ModifierOption : o) };
    }));
  };

  const deleteOption = (groupId: string, optId: string) => {
    setGroups(prev => prev.map(g => {
      if (g.id !== groupId) return g;
  ...
  const toggleProductMod = (productId: string, groupId: string) => {
    setProductMods(prev => {
      const next = { ...prev };
        ...
        {groups.map(g => (
          <div key={g.id} style={{ border: `1px solid ${C.line}`, background: C.surface }} className="rounded-xl overflow-hidden">
            ...
            <div className="flex items-center gap-2 px-3 py-2" style={{ background: C.surfaceLight }}>
              ...
              <Toggle value={g.required} onChange={v => updateGroup(g.id, 'required', v)} label="Obligatorio" color={C.brassLight} C={C} />
              ...
              {(g.options || []).map(o => (
                <div key={o.id} style={{ background: C.surfaceLight }} className="rounded-lg p-3">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <Toggle value={o.isDefault} onChange={v => updateOption(g.id, o.id, 'isDefault', v)} label="Por defecto" color={C.sageLight} C={C} />
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    }} label="Descuenta inventario" color={C.sageLight} C={C} />
                    {o.stockDeduct && (
                      <>
