...
export default function MenuDelDiaSelector({ menu, catalog, colors: C, onConfirm, onClose }) {
  const [selections, setSelections] = useState({});

  function toggleProduct(courseId, productId) {
    setSelections(s => {
        ...
        {menu.schedules && menu.schedules.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1">
            {menu.schedules.map((s, i) => {
              ...
              return (
                <span key={i} style={{ color: C.muted }} className="text-[9px] px-1.5 py-0.5 rounded"
                  style2={{ background: C.surfaceLight }}>
