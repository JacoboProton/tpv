...
function getShapeStyle(table, zones, C) {
...
export default function FloorEditor({ floor, persistFloor, colors: C }) {
  ...
  const selectedTable = floor.tables.find(t => t.id === selectedId);
  ...
  // Ensure all tables have floor-plan defaults
  ...
  useEffect(() => {
    const handleKey = (e) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        if (selectedId && !e.target.closest('input')) {
          e.preventDefault();
  ...
  function updateTableProp(id, key, value) {
    const next = clone(floor);
  ...
  function handleBackgroundUpload(e) {
    ...
    reader.onload = (ev) => {
      const next = clone(floor);
      next.background = { url: ev.target.result, opacity: 50 };
      persistFloor(next);
    };
    reader.readAsDataURL(file);
  }

  function handleZoneChange(tableId, zoneId) {
    pushUndo();
    const next = clone(floor);
    const t = next.tables.find(t => t.id === tableId);
    if (t) t.zone = zoneId;
  ...
  // Canvas pan via middle mouse / wheel
      ...
      {floor.background?.url && (
        <div className="flex items-center gap-3 mt-3" style={{ color: C.muted }}>
          ...
          <input type="range" min="5" max="100" value={floor.background.opacity ?? 50}
            onChange={e => {
              const next = clone(floor);
              next.background = { ...next.background, opacity: parseInt(e.target.value) };
              persistFloor(next);
            }}
