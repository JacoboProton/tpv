...
const ZONES = ['spain', 'eu', 'outside_eu'];
const ZONE_LABELS = { spain: 'España', eu: 'UE', outside_eu: 'Fuera de la UE' };
const LINE_TYPES = ['good', 'service'];
const TYPE_LABELS = { good: 'Bien', service: 'Servicio' };

export default function GestoriaView({ sales, colors: C }) {
  ...
  async function loadAll() {
    ...
    try {
      ...
      setSettings(s || { taxRegime: 'autonomo', criterionOfCash: 'false', socialSecurityRed: '' });
      setTaxModels(tx || []);
  ...
  async function handleCalculate(modelCode, year = new Date().getFullYear(), quarter = 1) {
    setCalculating(`${modelCode}-${year}-${quarter}`);
    try {
      const { data } = await import('../lib/api').then(m => m.calculateGestoriaTaxModel(modelCode, year, quarter));
      setTaxModels(prev => {
  ...
  function getModelStatus(code, year, quarter) {
    return taxModels.find(t => t.model_code === code && t.year === year && t.quarter === quarter) || null;
  }

  function downloadModelJSON(code, year, quarter) {
  ...
  function downloadModelPDF(code, year, quarter) {
    const m = getModelStatus(code, year, quarter);
    if (!m || !m.data) return;
  ...
  function downloadModelCSV(code, year, quarter) {
    const m = getModelStatus(code, year, quarter);
    if (!m || !m.data) return;
...
function PanelTab({ settings, taxModels, currentYear, calculating, getModelStatus, onCalculate, onDownloadPDF, onDownloadJSON, onDownloadCSV, C }) {
  const regimeLabel = { autonomo: 'Autónomo (Estimación Directa)', modulos: 'Módulos', sl: 'Sociedad (SL)' }[settings.taxRegime] || 'Autónomo';
...
function DocumentsTab({ type, title, docs, onDataChange, C }) {
  ...
  const totalBase = docs.reduce((s, d) => {
    ...
    return s + lines.reduce((sl, l) => sl + Number(l.baseAmount || l.base_amount || 0), 0);
  }, 0);
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && <DocumentForm type={type} onSave={async (doc) => {
...
function DocumentCard({ doc, type, onDelete, onToggleConfirm, C }) {
  ...
  const totalBase = lines.reduce((s, l) => s + Number(l.baseAmount || l.base_amount || 0), 0);
  const totalVat = lines.reduce((s, l) => s + Number(l.vatAmount || l.vat_amount || 0), 0);
...
function PayrollsTab({ payrolls, onDataChange, C }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && <PayrollForm onSave={async (p) => {
        ...
        {isAuthorized ? (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={async () => {
                const auth = await import('../lib/api').then(m => m.fetchGestoriaAuthorization());
...
function generateModelPDF(code, data, year, quarter) {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><style>
    ...
    ${nota ? `<div class="nota">⚠️ ${nota}</div>` : ''}
    <div class="footer">
