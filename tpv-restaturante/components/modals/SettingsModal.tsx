...
import type { TicketSettings, Category } from '@tpv/core'
...
interface Props {
  ...
  catalog: { categories?: Category[] | string[] } | null | undefined
}
...
export default function SettingsModal({ C, ticketSettings, setTicketSettings, showSettings, setShowSettings, showToast, catalog }: Props) {
  ...
  const patchSettings = (patches: Record<string, unknown>) => {
    setTicketSettings((s) => ({ ...s, ...patches }))
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 no-print" style={{ background: 'rgba(0,0,0,0.65)' }}>
      <div style={{ background: C.surface, border: `1px solid ${C.line}` }} className="w-full max-w-sm rounded-xl p-5 fade-up max-h-[85vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-3">
          {['restaurantName', 'companyCif', 'companyAddress', 'companyPhone', 'logoUrl', 'footerText', 'ticketWidth'].map((field: string) => (
            <div key={field}>
              ...
              <input
                value={String(ticketSettings[field] ?? '')}
          ...
          <select
            value={str(ticketSettings.drawerOpenPolicy, 'confirm')}
...
<p className="font-display text-sm" style={{ color: C.cream }}>Descuento de personal</p>
          ...
          {(() => {
            ...
            const cats = catalog?.categories?.map((c: Category | string) => typeof c === 'string' ? c : c.name) || []
            const allKeys = [...new Set([...cats, ...Object.keys(rates)])]
            return allKeys.map((catName: string) => (
          ...
          <ToggleRow C={C} label="Imprimir ticket al confirmar pedido público (QR/online)" value={ticketSettings.printPublicConfirmation} defaultValue="false" onChange={v => patchSettings({ printPublicConfirmation: v })} />
          <div style={{ borderTop: `1px solid ${C.line}` }} className="my-2" />
          <p className="font-display text-sm" style={{ color: C.cream }}>Pedido por QR</p>
          <ToggleRow C={C} label="Activar pedido QR en mesa" value={ticketSettings.qrOrderingEnabled} defaultValue="true" onChange={v => patchSettings({ qrOrderingEnabled: v })} />
          <ToggleRow C={C} label="Requerir pago al pedir" value={ticketSettings.qrRequirePayment} defaultValue="false" onChange={v => patchSettings({ qrRequirePayment: v })} />
          <div>
            <label className="text-[10px] uppercase tracking-wide mb-1 block" style={{ color: C.muted }}>Color primario QR</label>
            <input value={str(ticketSettings.qrThemePrimary, '#c4a04a')} onChange={e => setTicketSettings((s) => ({ ...s, qrThemePrimary: e.target.value }))}
              style={{ background: C.surfaceLight, color: C.cream }}
          ...
          <div>
            ...
            <input value={str(ticketSettings.qrThemeSecondary, '#1a1a1a')} onChange={e => setTicketSettings((s) => ({ ...s, qrThemeSecondary: e.target.value }))}
              style={{ background: C.surfaceLight, color: C.cream }}
          ...
          <ToggleRow C={C} label="Activar pedidos online" value={ticketSettings.onlineOrderingEnabled} defaultValue="true" onChange={v => patchSettings({ onlineOrderingEnabled: v })} />
          <ToggleRow C={C} label="Requerir pago online" value={ticketSettings.onlinePaymentRequired} defaultValue="true" onChange={v => patchSettings({ onlinePaymentRequired: v })} />
          <ToggleRow C={C} label="Aceptar automáticamente" value={ticketSettings.onlineAutoAccept} defaultValue="true" onChange={v => patchSettings({ onlineAutoAccept: v })} />
          <div>
            <label className="text-[10px] uppercase tracking-wide mb-1 block" style={{ color: C.muted }}>Tiempo preparación (min)</label>
            <input value={str(ticketSettings.onlinePrepTime, '20')} onChange={e => setTicketSettings((s) => ({ ...s, onlinePrepTime: e.target.value }))}
          ...
          <ToggleRow C={C} label="Activar fichaje de empleados" value={ticketSettings.clockinEnabled} defaultValue="true" onChange={v => patchSettings({ clockinEnabled: v })} />
          <ToggleRow C={C} label="Requerir PIN para fichar" value={ticketSettings.clockinPinRequired} defaultValue="true" onChange={v => patchSettings({ clockinPinRequired: v })} />
          <ToggleRow C={C} label="Geolocalización requerida" value={ticketSettings.clockinGeolocation} defaultValue="false" onChange={v => patchSettings({ clockinGeolocation: v })} />
        </div>
...
function ToggleRow({ C, label, value, defaultValue, onChange }: { C: Record<string, string>; label: string; value: unknown; defaultValue: string; onChange: (v: string) => void }) {
  const on = str(value, defaultValue) === 'true'
  return (
