...
describe('ensureCategoryExists', () => {
  it('adds category if missing', () => {
    const catalog: Catalog = { categories: [cat('Bebidas')], products: [] }
    const result = ensureCategoryExists(catalog, 'Cocina')
    expect(result.categories.map(c => c.name)).toContain('Cocina')
    expect(result).not.toBe(catalog)
  })

  it('returns same catalog if category exists', () => {
    const catalog: Catalog = { categories: [cat('Bebidas'), cat('Cocina')], products: [] }
...
describe('removeProduct', () => {
  it('removes product by id', () => {
    const catalog: Catalog = { categories: [], products: [{ id: 'p1' } as any, { id: 'p2' } as any, { id: 'p3' } as any] }
...
describe('toggleProductAgotado', () => {
  it('sets agotado to true', () => {
    const catalog: Catalog = { categories: [], products: [{ id: 'p1', name: 'Café' } as any, { id: 'p2', name: 'Té' } as any] }
  ...
  it('sets agotado to false', () => {
    const catalog: Catalog = { categories: [], products: [{ id: 'p1', name: 'Café', agotado: true } as any] }
...
describe('getProductImage', () => {
  it('returns image URL if product has one', () => {
    const catalog: Catalog = { categories: [], products: [{ id: 'p1', image: '/img/cafe.jpg' } as any] }
    expect(getProductImage(catalog, 'p1')).toBe('/img/cafe.jpg')
  })

  it('returns undefined if product has no image', () => {
    const catalog: Catalog = { categories: [], products: [{ id: 'p1' } as any] }
  ...
  it('returns undefined for null catalog', () => {
    expect(getProductImage(null as any, 'p1')).toBeUndefined()
  })
})
...
describe('detectStockChanges', () => {
  const oldCatalog: Catalog = {
    categories: [],
    products: [
      { id: 'p1', name: 'Café', stockByLocation: { Bar: { stock: 10, lowStock: 3 } } } as any,
    ],
  }
  ...
  it('detects stock decrease', () => {
    const newCatalog: Catalog = {
      categories: [],
      products: [
        { id: 'p1', name: 'Café', stockByLocation: { Bar: { stock: 7, lowStock: 3 } } } as any,
      ],
    }
  ...
  it('detects stock increase', () => {
    const newCatalog: Catalog = {
      categories: [],
      products: [
        { id: 'p1', name: 'Café', stockByLocation: { Bar: { stock: 15, lowStock: 3 } } } as any,
      ],
    }
    const deltas = detectStockChanges(oldCatalog, newCatalog, 'p1')
    expect(deltas).toHaveLength(1)
  ...
  it('detects new location added', () => {
    const newCatalog: Catalog = {
      categories: [],
      products: [
        { id: 'p1', name: 'Café', stockByLocation: { Bar: { stock: 10, lowStock: 3 }, Cocina: { stock: 5, lowStock: 2 } } } as any,
      ],
    }
    const deltas = detectStockChanges(oldCatalog, newCatalog, 'p1')
    expect(deltas).toHaveLength(1)
...
describe('addProductToCatalog', () => {
  it('adds product and ensures category exists', () => {
    ...
    expect(next.categories.map(c => c.name)).toContain('Bebidas')
  })
})

describe('setProductField', () => {
  it('updates a string field', () => {
    const catalog: Catalog = { categories: [], products: [{ id: 'p1', name: 'Vino', price: 5 } as any] }
  ...
  it('handles stockByLocation object', () => {
    const catalog: Catalog = { categories: [], products: [{ id: 'p1', name: 'Vino', price: 5, stockByLocation: { Bar: { stock: 5, lowStock: 2 } } } as any] }
  ...
  it('returns null if product not found', () => {
    const catalog: Catalog = { categories: [], products: [] }
...
describe('getLowStockProducts', () => {
  it('returns products with stock below lowStock', () => {
    const catalog: Catalog = {
      categories: [],
      products: [
        { id: 'p1', name: 'Vino', stockByLocation: { Bar: { stock: 1, lowStock: 5 } } } as any,
        { id: 'p2', name: 'Agua', stockByLocation: { Bar: { stock: 10, lowStock: 5 } } } as any,
      ],
    }
  ...
  it('returns empty array if no stockByLocation', () => {
    const catalog: Catalog = { categories: [], products: [{ id: 'p1', name: 'Vino' } as any] }
    expect(getLowStockProducts(catalog)).toHaveLength(0)
  })

  it('returns empty array for null catalog', () => {
    expect(getLowStockProducts(null as any)).toHaveLength(0)
  })
})
