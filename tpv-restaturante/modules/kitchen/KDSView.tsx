        ...
        <div className={layout === 'compact' ? 'space-y-1' : 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3'}>
          {filteredOrders.map(order => (
            <OrderCard key={order.orderId} order={order} now={now} layout={layout} K={K} KTC={KTC}
              ...
              onReprint={() => onReprint?.(order.orderId)} catalog={catalog} />
          ))}
        </div>
      </div>
    );
              ...
              <div key={`${group.orderId}`} className={`rounded-xl overflow-hidden ${flash ? 'animate-pulse' : ''}`}
                ...
                <div className="p-3 space-y-2">
                  {group.items.map(item => {
                    const img = catalog?.products?.find((p: KDSProduct) => p.id === item.productId)?.image;
                    return (
                      <button key={item.id} onClick={() => handleItemState(item.orderId, item.id)}
                        className="w-full text-left px-3 py-2.5 rounded-lg font-medium text-sm hover:opacity-80 transition-opacity flex items-center gap-2"
                        style={{ background: KTC.surfaceLight, color: KTC.cream }}>
                        {img && <img src={img} alt="" className="w-6 h-6 rounded object-cover shrink-0" />}
                        {item.qty}× {item.name}
                      </button>
                    );
                  })}
...
function OrderCard({ order, now, layout, K, KTC, onItemClick, onReprint, catalog }: OrderCardProps) {
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                {img && <img src={img} alt="" className="w-7 h-7 rounded object-cover shrink-0" />}
