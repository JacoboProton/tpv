...
export default function PublicWaitlistPage() {
  ...
  useEffect(() => {
    fetch('/api/settings').then(r => r.json()).then((s: { waitlistEnabled?: string; waitlistWelcomeMessage?: string }) => {
  ...
  async function loadQueue() {
    try {
      ...
      const data = await r.json() as QueueEntry[];
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    try {
      ...
      const data = await r.json() as { ok?: boolean; position?: number; id?: string; error?: string };
      if (data.ok) {
        setMessage(`¡Te has apuntado! Eres el nº ${data.position} en la cola.`);
        setMyEntry({ name, phone, pax, position: data.position || 0, id: data.id || '' });
        setMode('status');
      } else {
