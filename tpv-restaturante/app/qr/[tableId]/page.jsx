...
export default function QrMenuPage() {
  ...
  const [buffetSession, setBuffetSession] = useState(null);
  const [buffetConfig, setBuffetConfig] = useState(null);
  ...
  const isPaused = buffetConfig?.paused_until > Date.now();
  const pauseRemaining = isPaused ? buffetConfig.paused_until - Date.now() : 0;
  const cooldownUntil = isBuffet ? buffetSession.cooldown_until : 0;
  ...
  const roundCap = isBuffet
    ? (buffetSession.override_round_cap > 0 ? buffetSession.override_round_cap : (buffetConfig?.round_cap || 3))
    : 99;
  const totalPeople = isBuffet ? (buffetSession.adult_count + buffetSession.child_count + buffetSession.senior_count) : 1;
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (buffet?.session) {
        setBuffetSession(buffet.session);
        setBuffetConfig(buffet.config);
      }
    ...
    try {
      if (isBuffet) {
        ...
        if (data.roundId) {
          ...
          setShowCart(false);
          setBuffetSession(prev => ({ ...prev, round: data.round, cooldown_until: data.cooldownUntil }));
        } else {
        ...
        <div className="min-h-screen flex flex-col items-center justify-center p-6" style={{ background: bg }}>
          <div className="w-full max-w-md text-center space-y-6">
            ...
            <p className="text-sm" style={{ color: '#8a8275' }}>
              Ronda {orderResult.round} — la cocina está preparándolo.{inCooldown ? '' : ' Podrás pedir otra ronda en unos minutos.'}
            </p>
          ...
          <div style={{ background: theme.primary + '20', borderBottom: '1px solid ' + theme.primary + '40' }} className="px-4 py-2">
            <div className="flex items-center justify-between text-xs">
              <span style={{ color: theme.primary }} className="font-bold">
                🍽️ Buffet · Ronda {buffetSession.round + 1}
              </span>
              <span style={{ color: '#8a8275' }}>
                {buffetSession.adult_count + buffetSession.child_count + buffetSession.senior_count} comensales
                {buffetSession.child_count > 0 && ` · ${buffetSession.child_count} 🛝`}
                {buffetSession.senior_count > 0 && ` · ${buffetSession.senior_count} 👴`}
              </span>
            </div>
          </div>
        )}

        {/* Cooldown / Pause banners */}
        {isPaused && (
      ...
      {cartCount > 0 && !isPaused && (
        <div className="fixed bottom-0 left-0 right-0 z-30 p-4" style={{ background: bg + 'f2', borderTop: '1px solid #333', backdropFilter: 'blur(8px)' }}>
          <button onClick={() => setShowCart(true)}
            ...
            {isBuffet && <span>Ronda {buffetSession.round + 1}</span>}
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-40 flex flex-col" style={{ background: bg + 'f2', backdropFilter: 'blur(4px)' }}>
          <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: '#333' }}>
            ...
            <h2 className="text-sm font-bold" style={{ color: '#e8e0d4' }}>
              {isBuffet ? `Ronda ${buffetSession.round + 1}` : 'Tu pedido'}
            </h2>
