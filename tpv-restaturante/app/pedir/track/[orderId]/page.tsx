...
const STATUS_STEPS: Array<{ status: string; label: string; icon: React.ComponentType<{ className?: string; style?: React.CSSProperties }>; color: string }> = [
...
const ITEM_LABELS: string[] = ['Recibido', 'Preparando', 'Listo', 'Entregado'];
const ITEM_KEYS: string[] = ['sent', 'inPreparation', 'ready', 'served'];
    ...
    <div className="min-h-screen" style={{ background: '#1a1a1a' }}>
      <div className="max-w-md mx-auto px-4 py-6 space-y-6">
              ...
              <div key={s.status} className="flex items-center">
                <div className="flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${isCurrent ? 'scale-110' : ''}`}
                    ...
                    <IconComponent className="w-4 h-4" style={{ color: done ? s.color : '#555' }} />
                  </div>
        ...
        <div className="space-y-2">
          ...
          {items.map((item, i) => {
            const itemIdx = ITEM_KEYS.findIndex(k => item[k as keyof OrderItem]);
            return (
