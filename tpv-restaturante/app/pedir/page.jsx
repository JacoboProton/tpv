...
export default function OnlineOrderingPage() {
  ...
  const [step, setStep] = useState('menu'); // menu | checkout | result

  // Form state
  const [modality, setModality] = useState('delivery');
  ...
  const modes = useMemo(() => {
    try { return JSON.parse(settings?.onlineOrderingModes || '["delivery"]'); } catch { return ['delivery']; }
  }, [settings]);

  const schedules = useMemo(() => {
    try { return JSON.parse(settings?.onlineSchedules || '[]'); } catch { return []; }
  }, [settings]);
        ...
        <div className="w-full max-w-md text-center space-y-6">
          ...
          <div className="p-4 rounded-xl space-y-2 text-left" style={{ background: '#222' }}>
            ...
            <div className="flex justify-between text-sm font-bold pt-2" style={{ borderTop: '1px solid #333' }}>
              ...
              <span style={{ color: theme.primary }}>{(cartTotal + deliveryCost).toLocaleString('es-ES', { minimumFractionDigits: 2 })} €</span>
            </div>
          </div>
          <button onClick={() => router.push(`/pedir/track/${orderResult.orderId}`)}
  ...
  return (
    <div className="min-h-screen" style={{ background: bg, paddingBottom: '80px' }}>
      ...
      <div className="sticky top-0 z-30" style={{ background: bg + 'e6', borderBottom: '1px solid #333', backdropFilter: 'blur(8px)' }}>
        ...
        {/* Modality selector (only when cart is empty or in menu step) */}
