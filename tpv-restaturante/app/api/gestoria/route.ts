...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ error: e.errors || (e as Error).message }, { status: 400 });
    }
    const { action } = body;
...
function getDueDate(modelCode: any, year: any, quarter: any) {
  ...
  const dd = (deadlines as any)[quarter] || '01-30';
  ...
  switch (modelCode) {
    ...
    case '111': {
      ...
      return {
        ...
        total_remuneraciones: round2(payrolls.reduce((s: any, p: any) => s + Number(p.gross_amount || 0), 0)),
    ...
    case '349': {
      const euDocs = confirmedDocs.reduce((acc: any, d: any) => {
        const lines = typeof d.lines === 'string' ? JSON.parse(d.lines) : (d.lines || []);
        const euLines = lines.filter((l: any) => l.zone === 'eu');
        return acc.concat(euLines.map((l: any) => ({
          nif: d.provider_nif || '',
          name: d.provider_name || d.file_name || '',
          base: Number(l.base_amount || 0),
          operacion: l.type === 'service' ? 'servicio' : 'bien',
      ...
      return {
        ...
        total_operaciones: round2(euDocs.reduce((s: any, e: any) => s + e.base, 0)),
      };
    }
    case '347': {
      const providers = confirmedDocs.reduce((acc: any, d: any) => {
        ...
        const base = (() => {
          const lines = typeof d.lines === 'string' ? JSON.parse(d.lines) : (d.lines || []);
          return lines.reduce((s: any, l: any) => s + Number(l.base_amount || 0), 0);
        })();
    ...
    case '190': {
      return {
        ...
        total_remuneraciones: round2(payrolls.reduce((s: any, p: any) => s + Number(p.gross_amount || 0), 0)),
        retenciones: round2(payrolls.reduce((s: any, p: any) => s + Number(p.irpf_withholding || 0), 0)),
      };
    }
    case '180': {
      return {
