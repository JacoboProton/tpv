...
export async function GET() {
  try {
    const rows = await sql`SELECT * FROM employees ORDER BY role DESC, name`;
...
export async function PUT(req) {
  try {
    ...
    for (const e of employees) {
      queries.push(sql`
        INSERT INTO employees (id, name, pin, role, position, work_type, work_pct, dni, notes,
          personal_discount_enabled, monthly_limit, monthly_used, monthly_used_month,
          whatsapp_code, whatsapp_linked, created_at)
        VALUES (${e.id}, ${e.name}, ${e.pin || ''}, ${e.role || 'camarero'}, ${e.position || ''},
...
export async function POST(req) {
  try {
    ...
    if (action === 'generate-codes') {
      const employees = await sql`SELECT id, name FROM employees WHERE whatsapp_linked = false`;
      const codes = employees.map(e => {
        const code = Math.random().toString(36).slice(2, 8).toUpperCase();
        return { employeeId: e.id, name: e.name, code };
      });
      for (const c of codes) {
        await sql`UPDATE employees SET whatsapp_code = ${c.code} WHERE id = ${c.employeeId}`;
      }
    ...
    if (action === 'link-whatsapp') {
      ...
      const emp = await sql`SELECT id, name FROM employees WHERE whatsapp_code = ${code}`;
      if (emp.length === 0) return NextResponse.json({ error: 'Código inválido' }, { status: 404 });
      await sql`
        UPDATE employees SET whatsapp_linked = true, whatsapp_code = '' WHERE id = ${emp[0].id}
      `;
