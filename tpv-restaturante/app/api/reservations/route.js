...
function makeId() { return 'res_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 8); }
...
export async function GET(req) {
  try {
    ...
    if (recurring === '1') {
      const rows = await sql`SELECT * FROM reservation_recurring ORDER BY weekday, time`;
    ...
    let query = sql`SELECT * FROM reservations`;
    const conds = [];
...
export async function POST(req) {
  try {
    ...
    if (r.recurring) {
      const id = r.id || makeId().replace('res_', 'rec_');
      await sql`
        INSERT INTO reservation_recurring (id, name, weekday, time, pax, phone, notes, zone, table_id, active, created_at)
        VALUES (${id}, ${r.name}, ${r.weekday}, ${r.time}, ${r.pax}, ${r.phone || ''}, ${r.notes || ''}, ${r.zone || ''}, ${r.tableId || ''}, true, ${Date.now()})
    ...
    await sql`
      INSERT INTO reservations (id, date, time, pax, name, phone, email, status, zone, notes, table_id, customer_id, deposit_amount, deposit_paid, source, created_at, updated_at)
      VALUES (${id}, ${r.date}, ${r.time}, ${r.pax}, ${r.name}, ${r.phone || ''}, ${r.email || ''},
        ${r.status || 'pendiente'}, ${r.zone || ''}, ${r.notes || ''}, ${r.tableId || ''}, ${r.customerId || ''},
        ${r.depositAmount || 0}, ${r.depositPaid || false}, ${r.source || 'manual'}, ${Date.now()}, ${Date.now()})
      ON CONFLICT (id) DO UPDATE SET
    ...
    if (r.tableId && (r.status === 'confirmada' || r.status === 'sentada' || r.status === 'pendiente')) {
      await sql`UPDATE tables SET reserved_for = ${r.name} WHERE id = ${r.tableId}`;
    }
    if (r.tableId && (r.status === 'cancelada' || r.status === 'noshow')) {
      await sql`UPDATE tables SET reserved_for = '' WHERE id = ${r.tableId}`;
    }
    return NextResponse.json({ ok: true, id });
  } catch (err) {
    return NextResponse.json({ error: err.message }, { status: 500 });
...
export async function DELETE(req) {
  try {
    ...
    if (recurring) {
      await sql`DELETE FROM reservation_recurring WHERE id = ${id}`;
      return NextResponse.json({ ok: true });
    }
    const row = await sql`SELECT table_id FROM reservations WHERE id = ${id}`;
    if (row.length > 0 && row[0].table_id) {
      await sql`UPDATE tables SET reserved_for = '' WHERE id = ${row[0].table_id}`;
    }
