...
export async function GET(req) {
  try {
    ...
    let query = sql`SELECT * FROM purchase_orders`;
    const conds = [];
    ...
    for (const o of orders) {
      const lines = await sql`SELECT * FROM purchase_order_lines WHERE order_id = ${o.id} ORDER BY id`;
      result.push({
...
export async function POST(req) {
  try {
    ...
    if (action === 'create') {
      ...
      await sql`INSERT INTO purchase_orders (id, supplier_id, supplier_name, status, expected_date, notes, created_by, created_at)
        VALUES (${id}, ${supplierId}, ${supplierName}, 'draft', ${expectedDate || ''}, ${notes || ''}, ${createdBy || ''}, ${Date.now()})`;
      for (const line of lines || []) {
        await sql`INSERT INTO purchase_order_lines (order_id, product_id, product_name, quantity, price_per_unit, supplier_sku)
          VALUES (${id}, ${line.productId}, ${line.productName}, ${line.quantity}, ${line.pricePerUnit || 0}, ${line.supplierSku || ''})`;
      }
    ...
    if (action === 'update-status') {
      ...
      await sql`UPDATE purchase_orders SET status=${status}, updated_at=${Date.now()} WHERE id=${id}`;
    ...
    if (action === 'update-lines') {
      ...
      for (const line of lines || []) {
        await sql`INSERT INTO purchase_order_lines (order_id, product_id, product_name, quantity, price_per_unit, supplier_sku, received_qty)
          VALUES (${id}, ${line.productId}, ${line.productName}, ${line.quantity}, ${line.pricePerUnit || 0}, ${line.supplierSku || ''}, ${line.receivedQty || 0})`;
      }
      await sql`UPDATE purchase_orders SET updated_at=${Date.now()} WHERE id=${id}`;
      return NextResponse.json({ ok: true });
    }
    ...
    if (action === 'receive') {
      ...
      for (const l of lines || []) {
        await sql`UPDATE purchase_order_lines SET received_qty=${l.receivedQty || 0} WHERE id=${l.lineId} AND order_id=${id}`;
      }
      // Record price history from received items
      const order = (await sql`SELECT * FROM purchase_orders WHERE id=${id}`)[0];
      for (const l of lines || []) {
        const line = (await sql`SELECT * FROM purchase_order_lines WHERE id=${l.lineId} AND order_id=${id}`)[0];
        if (line && l.receivedQty > 0) {
          const cat = await sql`
            SELECT sc.id FROM supplier_catalog sc
            WHERE sc.supplier_id = ${order.supplier_id} AND sc.product_id = ${line.product_id} LIMIT 1
          `;
          if (cat.length > 0) {
            ...
            await sql`INSERT INTO supplier_price_history (catalog_id, supplier_id, product_id, pack_price, pack_size, price_per_unit, source, created_at)
              VALUES (${cat[0].id}, ${order.supplier_id}, ${line.product_id}, ${ppu}, 1, ${ppu}, 'receipt', ${Date.now()})`;
          }
        }
      }
      const allLines = await sql`SELECT quantity, received_qty FROM purchase_order_lines WHERE order_id=${id}`;
      ...
      await sql`UPDATE purchase_orders SET status=${newStatus}, updated_at=${Date.now()} WHERE id=${id}`;
...
async function handleAutoPreview(body) {
  ...
  const products = await sql`
    SELECT p.id, p.name, p.type,
      COALESCE((SELECT SUM(ps.stock) FROM product_stock ps WHERE ps.product_id = p.id), 0) AS total_stock
  ...
  const toReplenish = products.filter(p => {
    ...
    const estimatedDailyConsumption = estimateDailyConsumption(p.id);
  ...
  for (const prod of toReplenish) {
    ...
    const dailyConsumption = estimateDailyConsumption(prod.id);
    ...
    let offers = await sql`
      ...
      WHERE sc.product_id = ${prod.id} AND sc.active = true AND sc.is_preferred = true
    ...
    if (offers.length === 0) {
      offers = await sql`
        SELECT sc.*, s.name AS supplier_name FROM supplier_catalog sc
        JOIN suppliers s ON s.id = sc.supplier_id
        WHERE sc.product_id = ${prod.id} AND sc.active = true
        ORDER BY sc.price LIMIT 1
      `;
    }
...
async function handleAutoGenerate(body) {
  ...
  for (const group of preview.preview) {
    const id = 'po_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6);
    await sql`INSERT INTO purchase_orders (id, supplier_id, supplier_name, status, created_by, notes, created_at)
      VALUES (${id}, ${group.supplierId}, ${group.supplierName}, 'draft', ${body.createdBy || 'auto'}, 'Pedido automático', ${Date.now()})`;
    for (const line of group.lines) {
      await sql`INSERT INTO purchase_order_lines (order_id, product_id, product_name, quantity, price_per_unit, supplier_sku)
        VALUES (${id}, ${line.productId}, ${line.productName}, ${line.quantity}, ${line.pricePerUnit}, ${line.supplierSku})`;
    }
...
async function estimateDailyConsumption(productId) {
  try {
    const thirtyDaysAgo = Date.now() - 30 * 86400000;
    const logs = await sql`
      SELECT SUM(ABS(change_amount)) AS total FROM stock_log
      WHERE product_id = ${productId} AND reason = 'venta' AND created_at >= ${thirtyDaysAgo}
    `;
