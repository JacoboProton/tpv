...
export async function POST(req) {
  try {
    ...
    if (action === 'save') {
      ...
      if (isPreferred) {
        await sql`UPDATE supplier_catalog SET is_preferred = false WHERE product_id = ${productId} AND tenant_id = ${tenantId} AND is_preferred = true`;
      }
      ...
      if (id) {
        ...
        await sql`INSERT INTO supplier_catalog (supplier_id, product_id, sku, price, pack_size, min_order, delivery_days, is_preferred, active, tenant_id)
      ...
      if (id || !id) {
        const catId = id || (await sql`SELECT id FROM supplier_catalog WHERE supplier_id=${supplierId} AND product_id=${productId} AND tenant_id = ${tenantId} LIMIT 1`)[0]?.id;
        if (catId) {
          ...
          await sql`INSERT INTO supplier_price_history (catalog_id, supplier_id, product_id, pack_price, pack_size, price_per_unit, source, created_at, tenant_id)
            VALUES (${catId}, ${supplierId}, ${productId}, ${price}, ${packSize || 1}, ${ppu}, 'manual', ${Date.now()}, ${tenantId})`;
        }
      }
    ...
    if (action === 'delete') {
      const cat = await sql`SELECT supplier_id, product_id, is_preferred FROM supplier_catalog WHERE id=${body.id} AND tenant_id = ${tenantId}`;
      await sql`DELETE FROM supplier_catalog WHERE id=${body.id} AND tenant_id = ${tenantId}`;

      if (cat.length > 0 && cat[0].is_preferred) {
        // Auto-promote another active offer
        const next = await sql`
          SELECT id FROM supplier_catalog WHERE product_id = ${cat[0].product_id} AND active = true AND tenant_id = ${tenantId}
          ORDER BY price LIMIT 1
        `;
        if (next.length > 0) {
          await sql`UPDATE supplier_catalog SET is_preferred = true WHERE id = ${next[0].id} AND tenant_id = ${tenantId}`;
        }
      }
      return NextResponse.json({ ok: true });
    }
