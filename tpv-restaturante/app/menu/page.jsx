...
function isQrVisible(p, categories) {
        ...
        {combos.length > 0 && (
          <section className="mt-12">
            ...
            <div className="px-6 grid gap-4">
                ...
                // Estimate min individual price for savings calculation
                let minIndividual = 0;
                if (hasSlots) {
                              ...
                              {slot.items.map(item => {
                                ...
                                return (
                                  <span key={item.id} className="bg-[#1a1d23] rounded-lg px-2 py-1 text-[11px] text-white/60 flex items-center gap-1">
                                    {p.name}
                                    {item.surcharge > 0 && <span className="text-[#b05e5e]">+{euros(item.surcharge)}</span>}
                                  </span>
                                );
                              })}
                            </div>
                        ...
                        {(combo.items || []).map((item, ii) => {
                          ...
                          return (
                            <div key={ii} className="bg-[#1a1d23] rounded-lg px-2.5 py-1.5 text-[11px] text-white/60 flex items-center gap-1">
                              ...
                              {item.quantity > 1 && <span className="text-[#c4a04a] font-mono">x{item.quantity}</span>}
                              {p.name}
                            </div>
                          );
                        })}
